...
public class TodoController {
	...
	private static final String BASE_URL = "http://localhost:8080/users/afreen/todos";
	...
	public ModelAndView saveTodo(@ModelAttribute("todos") Todo theTodo) {
		...
		theTodo.setUsername("afreen");
		restTemplate.postForObject(BASE_URL, theTodo,Todo.class);
		return modelAndView;
	}
	
	@GetMapping(path="users/todos/showFormForUpdate")
	public ModelAndView updateTodo(@RequestParam("todoId") Long theId, Model theModel) {
	...
	public ModelAndView deleteTodo(@RequestParam("todoId") Long theId) {
		Map<String, Long> params = new HashMap<>();
		params.put("theId", theId);
